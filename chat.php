...
function route()
{
	...
	if (!isset($_REQUEST['action'])) {
		...
		error_log("[POST DEBUG 1] Starting post action");
		try {
			check_session();
			error_log("[POST DEBUG 2] Session checked - user: {$U['nickname']}, status: {$U['status']}");
		} catch (Exception $e) {
			error_log("[POST DEBUG ERROR] Session check failed: " . $e->getMessage());
			throw $e;
		}
		
		// Convert Rust client sendto codes to PHP expected values
		if (isset($_REQUEST['sendto'])) {
			$original_sendto = $_REQUEST['sendto'];
			if ($_REQUEST['sendto'] === 's *') {
				$_REQUEST['sendto'] = 's 17'; // All (broadcast)
			} elseif ($_REQUEST['sendto'] === 's ?') {
				$_REQUEST['sendto'] = 's 31'; // Members channel
			} elseif ($_REQUEST['sendto'] === 's %') {
				$_REQUEST['sendto'] = 's 48'; // Staff channel
			} elseif ($_REQUEST['sendto'] === 's _') {
				$_REQUEST['sendto'] = 's 56'; // Admin channel
			}
			if ($original_sendto !== $_REQUEST['sendto']) {
				error_log("[POST DEBUG 3] Converted sendto: '{$original_sendto}' -> '{$_REQUEST['sendto']}'");
			} else {
				error_log("[POST DEBUG 3] No sendto conversion needed: '{$_REQUEST['sendto']}'");
			}
		} else {
			error_log("[POST DEBUG 3] No sendto parameter set");
		}
		...
		if (isset($_REQUEST['kick']) && isset($_REQUEST['sendto']) && $_REQUEST['sendto'] !== ('s 48' || 's 56')) {
			...
			error_log("[POST DEBUG 4] Message parameter detected: '" . substr($_REQUEST['message'], 0, 50) . "'");
			// Set default sendto if not provided (This Room)
			if (!isset($_REQUEST['sendto']) || $_REQUEST['sendto'] === '') {
				$_REQUEST['sendto'] = 'room';
				error_log("[POST DEBUG 5] Set default sendto='room'");
			} else {
				error_log("[POST DEBUG 5] Using sendto='" . $_REQUEST['sendto'] . "'");
			}
			try {
				error_log("[POST DEBUG 6] Calling validate_input()");
				$rejected = validate_input();
				error_log("[POST DEBUG 7] validate_input() completed, rejected='" . ($rejected ? $rejected : 'none') . "'");
				send_post($rejected);
			} catch (Exception $e) {
				error_log("[POST DEBUG ERROR] validate_input() failed: " . $e->getMessage() . " at " . $e->getFile() . ":" . $e->getLine());
				error_log("[POST DEBUG ERROR] Stack trace: " . $e->getTraceAsString());
				send_error("Failed to post message: " . $e->getMessage());
			}
		} else {
			error_log("[POST DEBUG 4] No message parameter");
		}
		send_post();
...
{
	// Minimal CSS for Rust client - just prevent overflow and separate messages
	echo '<style type="text/css">';
	echo 'body{background-color:#000000;color:#FFFFFF;font-size:14px;word-wrap:break-word;margin:0;padding:0;} ';
	echo 'input,select,textarea{background-color:#111;color:#fff;border:1px solid #333;} ';
	echo '#messages{overflow-x:hidden;max-width:100%;margin-right:220px;} ';
	echo '#messages .msg a{color: #47aae7ff;text-decoration: underline dotted #47aae7ff;} ';
	echo '.msg{border-bottom:1px solid #333;padding:5px 10px;margin:5px 0;} ';
	echo '#topic{padding:10px;margin:10px 0;border-bottom:1px solid #333;} ';
	echo '#chatters{position:sticky;top:0;float:right;width:200px;max-height:100vh;overflow-y:auto;overflow-x:hidden;background-color:#000;} ';
	echo '#chatters table{border-spacing:0px;width:100%;} ';
	echo '#chatters th{position:sticky;top:0;display:block;padding:2px 5px;margin-top:10px;font-weight:bold;border-bottom:1px solid #333;text-align:left;background-color:#000;z-index:1;white-space:nowrap;} ';
	echo '#chatters th:first-child{margin-top:0;} ';
	echo '#chatters th a{display:inline;padding:0;} ';
	echo '#chatters td{display:block;padding:2px 5px;text-align:left;} ';
	echo '#chatters a{text-decoration:none;display:block;padding:2px 0;} ';
	echo 'iframe{width:100%;height:100%;margin:0;padding:0;border:none;display:block;} ';
	echo '.post table,.controls table{border-spacing:0;margin:0 auto;} ';
	echo '.post input,.post select,.post textarea{background-color:#111;color:#fff;border:1px solid #333;padding:3px;} ';
	echo '.post textarea{width:90%;height:60px;} ';
	echo 'input[type="submit"]{background-color:#222;color:#fff;border:1px solid #555;padding:5px 10px;cursor:pointer;} ';
	echo 'input[type="submit"]:hover{background-color:#333;} ';
	echo '.login{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;} ';
	echo '.login h2{margin-bottom:30px;} ';
	echo '.login form{border:1px solid #333;padding:20px;max-width:400px;width:100%;} ';
	echo '.login input[type="text"],.login input[type="password"],.login select{width:100%;padding:8px;box-sizing:border-box;margin-bottom:15px;background-color:#111;color:#fff;border:1px solid #333;} ';
	echo '.captcha-row{display:flex;gap:10px;align-items:center;margin-bottom:15px;} ';
	echo '.captcha-row img{flex-shrink:0;} ';
	echo '.captcha-row input{flex:1;padding:8px;background-color:#111;color:#fff;border:1px solid #333;} ';
	echo '.profile,.notes,.help{padding:20px;max-width:900px;margin:0 auto;} ';
	echo '.profile{text-align:center;} ';
	echo '.profile h1,.profile h2,.notes h1,.notes h2,.help h1,.help h2{border-bottom:1px solid #333;padding-bottom:10px;margin-bottom:20px;} ';
	echo '.profile table{border-spacing:0;margin:10px 0;display:inline-block;} ';
	echo '.profile td,.profile th{padding:3px 8px;text-align:left;} ';
	echo '.profile th{font-weight:bold;} ';
	echo '.profile input,.profile select,.profile textarea{background-color:#111;color:#fff;border:1px solid #333;padding:5px;} ';
	echo '.admin{padding:15px;max-width:800px;margin:0 auto;text-align:center;} ';
	echo '.admin h2{margin:20px 0 10px 0;font-size:16px;} ';
	echo '.admin table{border-spacing:0;margin:10px 0;display:inline-block;} ';
	echo '.admin td{padding:3px 8px;} ';
	echo '.admin input,.admin select,.admin textarea{background-color:#111;color:#fff;border:1px solid #333;padding:4px;} ';
	echo '.notes textarea{width:100%;min-height:400px;background-color:#111;color:#fff;border:1px solid #333;padding:10px;box-sizing:border-box;font-family:monospace;} ';
	echo '.help{line-height:1.6;} ';
	echo '.help a{color:#4798e7;text-decoration:none;} ';
	echo '.help a:hover{text-decoration:underline;} ';
	echo '.help code{background-color:#111;padding:2px 5px;border:1px solid #333;font-family:monospace;} ';
	echo '.help pre{background-color:#111;padding:10px;border:1px solid #333;overflow-x:auto;} ';
	echo '</style>';
}
...
{
	global $language;
	$form = "<form action=\"/chat/min\" enctype=\"multipart/form-data\" method=\"post\">" . hidden('lang', $language) . hidden('nc', substr(time(), -6)) . hidden('action', $action);
...
{
	global $language;
	$form = "<form action=\"/chat/min\" enctype=\"multipart/form-data\" method=\"post\" target=\"$target\">" . hidden('lang', $language) . hidden('nc', substr(time(), -6)) . hidden('action', $action);
	if (!empty($_REQUEST['session'])) {
		$form .= hidden('session', $_REQUEST['session']);
	}
	if ($do !== '') {
...
function print_start($class = '', $ref = 0, $url = '')
{
	...
	echo '<meta http-equiv="onion-location" content="http://4o4o4hn4hsujpnbsso7tqigujuokafxys62thulbk2k3mf46vq22qfqd.onion/chat/min" />';
...
function send_captcha()
{
	...
	echo hidden('challenge', $randid) . '<input type="text" name="captcha" placeholder="Enter code" autocomplete="off"></div>';
}
...
function send_setup($C)
{
    ...
    */
	...
	if ($U['status'] >= 6) {
		...
		echo '<tr><td><table id="bridge_mappings"><tr><th colspan="3">IRC Bridge Channel Mappings</th></tr>';
		echo '<tr><th>PHP Destination</th><th>IRC Channel</th><th>Action</th></tr>';

		// Fetch existing mappings
		$mappings = $db->query('SELECT destination_key, irc_channel FROM ' . PREFIX . 'bridge_mappings ORDER BY destination_key');
		while ($mapping = $mappings->fetch(PDO::FETCH_ASSOC)) {
			echo '<tr>';
			echo '<td>' . htmlspecialchars($mapping['destination_key']) . '</td>';
			echo '<td>' . htmlspecialchars($mapping['irc_channel']) . '</td>';
			echo '<td><button type="button" onclick="if(confirm(\'Delete mapping?\')) { var form = document.createElement(\'form\'); form.method = \'POST\'; form.action = \'?action=setup&do=save\'; var input1 = document.createElement(\'input\'); input1.name = \'delete_mapping\'; input1.value = \'' . htmlspecialchars($mapping['destination_key']) . '\'; form.appendChild(input1); var input2 = document.createElement(\'input\'); input2.name = \'session\'; input2.value = \'' . $U['session'] . '\'; form.appendChild(input2); document.body.appendChild(form); form.submit(); }">Delete</button></td>';
			echo '</tr>';
		}

		// Add new mapping row
		echo '<tr><td colspan="3"><strong>Add New Mapping:</strong></td></tr>';
		echo '<tr>';
		echo '<td><input type="text" name="new_mapping_dest" placeholder="room:*, room:6, s 31" size="20"></td>';
		echo '<td><input type="text" name="new_mapping_channel" placeholder="#channelname" size="20"></td>';
		echo '<td><small>Leave blank to skip</small></td>';
		echo '</tr>';
		echo '<tr><td colspan="3"><small><strong>Examples:</strong><br>';
		echo '• <code>room:*</code> → <code>#main</code> (Main chat room)<br>';
		echo '• <code>room:6</code> → <code>#room6</code> (Specific room ID)<br>';
		echo '• <code>s 31</code> → <code>#members</code> (Members channel)<br>';
		echo '• <code>s 48</code> → <code>#staff</code> (Staff channel)<br>';
		echo '• <code>s 58</code> → <code>#admin</code> (Admin channel)</small></td></tr>';
		echo '</table></td></tr>';
		thr();
	}
...
function send_user_history($user) {
	...
	echo "<h2 style='margin:0;font-size:24px;color:#fff;'>User Dashboard: " . htmlspecialchars($user) . "</h2>";
	...
	if ($show_quick_actions) {
		...
		echo "<h3 style='margin:0 0 12px 0;font-size:15px;font-weight:600;'>Quick Actions</h3>";
		if ($U['status'] >= 3 && $U['status'] < 5 && !is_staff_online()) {
			echo "<div style='color:#ff9900;font-size:12px;margin-bottom:12px;padding:8px;background:#1a0f00;border-radius:4px;'>⚠️ Emergency moderation enabled</div>";
		}
		...
		echo '<input type="text" name="reason" placeholder="Warning reason..." required style="flex:1;min-width:250px;padding:10px 14px;background:#000;border:1px solid #333;color:#fff;border-radius:5px;font-size:14px;">';
		...
		echo '<label style="font-size:13px;white-space:nowrap;display:flex;align-items:center;gap:4px;"><input type="checkbox" name="no_expire" value="1"> Permanent</label>';
		echo submit('⚠ Warn');
		echo '</div>';
		echo '</form>';
		...
		echo '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">';
		echo '<input type="number" name="duration" placeholder="Min" min="1" max="10080" required style="width:90px;padding:10px 14px;background:#000;border:1px solid #333;color:#fff;border-radius:5px;font-size:14px;">';
		echo '<input type="text" name="reason" placeholder="Mute reason..." required style="flex:1;min-width:250px;padding:10px 14px;background:#000;border:1px solid #333;color:#fff;border-radius:5px;font-size:14px;">';
		...
		echo submit('✓ Clear All Warnings');
		echo '</form>';
...
function send_frameset()
{
	...
	// }
	$nc = substr(time(), -6);
	echo "<div id=\"frameset-mid\" style=\"$frameset_mid_style\"><iframe name=\"view\" src=\"?action=$action_mid&session=$U[session]&lang=$language&nc=$nc$bottom\">" . noframe_html() . "</iframe></div>";
	echo "<div id=\"frameset-top\" style=\"$frameset_top_style\"><iframe $noscroll_top name=\"$action_top\" src=\"?action=$action_top&session=$U[session]&lang=$language&nc=$nc\">" . noframe_html() . "</iframe></div>";
	echo "<div id=\"frameset-bot\" style=\"$frameset_bot_style\"><iframe $noscroll_bot name=\"$action_bot\" src=\"?action=$action_bot&session=$U[session]&lang=$language&nc=$nc$sort_bot\">" . noframe_html() . "</iframe></div>";
	echo '</body></html>';
	exit;
}
...
function send_messages()
{
	...
	// Hidden form for Rust client to extract nc and postid values
	echo '<form style="display:none;">';
	echo '<input type="hidden" name="nc" value="' . substr(time(), -6) . '">';
	echo '<input type="hidden" name="postid" value="' . substr(time(), -6) . '">';
	echo '</form>';
...
function send_login()
{
	...
	print_start('login');
	$englobal = (int) get_setting('englobalpass');
	echo '<h1>404 Chatroom Not Found</h1>';
    echo '<p>(Minimal Edition)</p>';
	echo '<h2>Login</h2>';
	...
	if ($englobal !== 1 || (isset($_REQUEST['globalpass']) && $_REQUEST['globalpass'] == get_setting('globalpass'))) {
		echo "<input type=\"text\" name=\"nick\" placeholder=\"Nickname\" autofocus>";
		echo "<input type=\"password\" name=\"pass\" placeholder=\"Password\">";
		send_captcha();
		
		if ($ga !== 0) {
			if (get_setting('guestreg') != 0) {
				echo "<input type=\"password\" name=\"regpass\" placeholder=\"Register Password (Optional)\">";
			}
			if ($englobal === 2) {
				echo "<input type=\"password\" name=\"globalpass\" placeholder=\"Global Password\">";
			}
			echo "<select name=\"colour\"><option value=\"\">* Random Color *</option>";
			print_colours();
			echo '</select>';
		} else {
			echo "<p>$I[noguests]</p>";
		}
		echo '<div>' . submit($I['enter']) . '</div></form>';
	} else {
		echo "<p>Global Password: <input type=\"password\" name=\"globalpass\" size=\"20\" autofocus></p>";
		if ($ga === 0) {
			echo "<p>$I[noguests]</p>";
		}
		echo '<p>' . submit($I['enter']) . '</p></form>';
	}
...
function print_chatters()
{
	...
	if (!$U['hidechatters']) {
		...
		// Start table structure for Rust client compatibility
		echo '<table><tr>';
		...
		while ($user = $stmt->fetch(PDO::FETCH_BOTH)) {
			...
			if ($U['status'] >= 5) {    //if logged in user is moderator or higher            
				...
				if ($user[2] >= 5) {
					...
					$M[] = $link; // Non-staff see all staff as staff
				} elseif ($user[2] == 3) {
		...
		// Output table structure with th/td pairs in SINGLE tr for Rust client
		if ($U['status'] >= 5) {
			$chanlinka = "<a style=\"color:#fff; text-decoration: none\" href=\"?action=post&amp;session=$U[session]&amp;lang=$language&amp;nc=$nc&amp;sendto=s 56\" target=\"post\">Admin</a>";
		...
		// Output alternating th/td pairs in single tr for Rust client compatibility
		// MUST output all 4 categories even if empty for proper th_count alignment
		// Rust expects: <tr><th>Admin</th><td>...</td><th>Staff</th><td>...</td><th>Members</th><td>...</td><th>Guests</th><td>...</td></tr>
		
		// Admin (th_count=1)
		echo "<th>" . $chanlinka . " (" . count($A) . ")</th>";
		echo "<td>" . implode("\n", $A) . "\n</td>";
		
		// Staff (th_count=2)
		echo "<th>" . $chanlinks . " (" . count($S) . ")</th>";
		echo "<td>" . implode("\n", $S) . "\n</td>";
		
		// Members (th_count=3)
		echo "<th>" . $chanlinkm . " (" . count($M) . ")</th>";
		echo "<td>" . implode("\n", $M) . "\n</td>";
		
		// Guests (th_count=4)
		echo "<th>" . $chanlinkg . " (" . count($G) . ")</th>";
		echo "<td>" . implode("\n", $G) . "\n</td>";
...
function kick_chatter($names, $mes, $purge)
{
	...
	foreach ($names as $name) {
		...
		if ($temp = $check->fetch(PDO::FETCH_ASSOC)) {
			...
			// Log the kick action with severity
			$kick_details = ($mes ?: 'No reason provided') . ' [Severity: 2] [Duration: ' . get_setting('kickpenalty') . ' min]';
			log_user_action($name, 'kick', $U['nickname'], $kick_details, get_setting('kickpenalty') * 60);
...
function validate_input()
{
	...
	error_log("[VALIDATE DEBUG 1] Starting validate_input for user: {$U['nickname']}");
	$inbox = false;
	...
	error_log("[VALIDATE DEBUG 2] Message length: " . strlen($message) . ", maxmessage: $maxmessage");
	...
	$recipient = '';
	error_log("[VALIDATE DEBUG 3] After processing, message='" . substr($message, 0, 50) . "'");

	// Parse /pm command from Rust client (bhcli.rs)
	// Format: /pm username message text
	if (preg_match('/^\/pm\s+([^\s]+)\s+(.+)$/i', $message, $pm_matches)) {
		error_log("[VALIDATE DEBUG 3.5] Detected /pm command: recipient='{$pm_matches[1]}', message='{$pm_matches[2]}'");
		$recipient = $pm_matches[1];
		$message = $pm_matches[2];
		// Override sendto to use the recipient from the /pm command
		$_REQUEST['sendto'] = $recipient;
	}
	...
	*/
	...
	// Members channel [M] - status 3+ (broadcasts to all rooms)
		...
		$allrooms = 1;
		// log_audit($U['nickname'], $U['status'], 'message_sent', null, null, 'Members Channel');
	...
	// Staff channel [Staff] - status 5+ (broadcasts to all rooms)
		...
		$roomid = null;
		$allrooms = 1;
		// log_audit($U['nickname'], $U['status'], 'message_sent', null, null, 'Staff Channel');
	...
	// Admin channel [Admin] - status 6+ (broadcasts to all rooms)
		...
		$roomid = null;
		$allrooms = 1;
		// log_audit($U['nickname'], $U['status'], 'message_sent', null, null, 'Admin Channel');
		
	// All (broadcast to everyone in all rooms) - status 5+
	} elseif ($_REQUEST['sendto'] === 's 17' && $U['status'] >= 5) {
		error_log("[VALIDATE DEBUG 4] Matched: All (Broadcast), user status={$U['status']}");
	...
	// System Message (backward compatibility) - status 5+
		...
		error_log("[VALIDATE DEBUG 4] Matched: System Message");
		$poststatus = 1;
	...
	// Check if sendto is a channel the user doesn't have permission for - fallback to This Room
	} elseif (in_array($_REQUEST['sendto'], ['s 17', 's 31', 's 48', 's 56', 's 50'])) {
		error_log("[VALIDATE DEBUG 4] Channel permission denied for '{$_REQUEST['sendto']}', status={$U['status']}, falling back to This Room");
		$poststatus = 1;
		$displaysend = style_this_clickable(htmlspecialchars($U['nickname']), $U['style']) . ' - ';
		$roomid = $U['roomid']; // Message stays in current room
		$allrooms = 0;
		// log_audit($U['nickname'], $U['status'], 'message_sent', null, null, 'This Room (fallback)');
		
	} else { // known nick in room?
		error_log("[VALIDATE DEBUG 4] Checking for PM recipient: sendto='" . $_REQUEST['sendto'] . "'");
	...
	error_log("[VALIDATE DEBUG 5] About to call add_message: poststatus=$poststatus, recipient='$recipient', allrooms=" . ($allrooms ?? 'not set'));
	try {
		$add_result = add_message($message, $recipient, $U['nickname'], $U['status'], $poststatus, $displaysend, $U['style']);
		error_log("[VALIDATE DEBUG 6] add_message returned: " . ($add_result ? 'true' : 'false'));
	} catch (Exception $e) {
		error_log("[VALIDATE DEBUG ERROR] add_message threw exception: " . $e->getMessage() . " at " . $e->getFile() . ":" . $e->getLine());
		error_log("[VALIDATE DEBUG ERROR] Stack trace: " . $e->getTraceAsString());
		throw $e;
	}
	if ($add_result) {
		$U['lastpost'] = time();
...
{
	global $db, $U;
	error_log("[ADD_MSG DEBUG 1] Starting add_message for poster='$poster', poststatus=$poststatus");
	
	if ($message === '') {
		error_log("[ADD_MSG DEBUG ERROR] Message is empty, returning false");
		return false;
	}
	
	//Modifications for chat rooms
	$roomid = $U['roomid'];
	error_log("[ADD_MSG DEBUG 2] Initial roomid={$U['roomid']}");
	
	// Check if message should broadcast to all rooms
	if (isset($_REQUEST['sendto'])) {
		$sendto = $_REQUEST['sendto'];
		error_log("[ADD_MSG DEBUG 3] sendto='$sendto'");
		// Broadcast to all rooms for: All (s 17), Members (s 31), Staff (s 48), Admin (s 56), System (s 50), or legacy (r @)
		if (in_array($sendto, ['s 17', 's 31', 's 48', 's 56', 's 50']) || ($sendto === 'r @' && $U['status'] >= 5)) {
			$allrooms = 1;
			$roomid = null;
			error_log("[ADD_MSG DEBUG 4] Broadcasting to all rooms (allrooms=1, roomid=null)");
		} else {
			$allrooms = 0;
			error_log("[ADD_MSG DEBUG 4] Staying in current room (allrooms=0)");
		}
	} else {
		$allrooms = 0;
		error_log("[ADD_MSG DEBUG 3] No sendto set, using default (allrooms=0)");
	}
	...
	error_log("[ADD_MSG DEBUG 5] messageText created, length=" . strlen($messageText));
	...
	error_log("[ADD_MSG DEBUG 6] newmessage array created with roomid=" . ($roomid ?? 'null') . ", allrooms=$allrooms");
	//Modifcation chat rooms
	if ($newmessage['roomid'] === NULL) {
		error_log("[ADD_MSG DEBUG 7] Checking for duplicate (roomid=NULL)");
		...
		error_log("[ADD_MSG DEBUG 7] Checking for duplicate (roomid={$newmessage['roomid']})");
	...
	if ($stmt->fetch(PDO::FETCH_NUM)) {
		error_log("[ADD_MSG DEBUG 8] Duplicate message detected, returning false");
		return false;
	}
	
	error_log("[ADD_MSG DEBUG 9] No duplicate found, calling write_message");
	try {
		write_message($newmessage);
		error_log("[ADD_MSG DEBUG 10] write_message completed successfully");
	} catch (Exception $e) {
		error_log("[ADD_MSG DEBUG ERROR] write_message threw exception: " . $e->getMessage());
		throw $e;
	}
	return true;
}
...
{
	global $db;
	error_log("[WRITE_MSG DEBUG 1] Starting write_message: poster='{$message['poster']}', poststatus={$message['poststatus']}, allrooms={$message['allrooms']}");
	
	if (MSGENCRYPTED) {
		$message['text'] = base64_encode(sodium_crypto_aead_aes256gcm_encrypt($message['text'], '', AES_IV, ENCRYPTKEY));
		error_log("[WRITE_MSG DEBUG 2] Message encrypted");
	}
	
	// Insert message with retry logic for database locks
	try {
		error_log("[WRITE_MSG DEBUG 3] Preparing SQL INSERT");
		$stmt = $db->prepare('INSERT INTO ' . PREFIX . 'messages (postdate, poststatus, poster, recipient, text, delstatus, roomid, allrooms) VALUES (?, ?, ?, ?, ?, ?, ?, ?);');
		error_log("[WRITE_MSG DEBUG 4] Executing INSERT with params: postdate={$message['postdate']}, poststatus={$message['poststatus']}, poster={$message['poster']}, recipient={$message['recipient']}, delstatus={$message['delstatus']}, roomid=" . ($message['roomid'] ?? 'null') . ", allrooms={$message['allrooms']}");
		...
		error_log("[WRITE_MSG DEBUG 5] INSERT successful, message ID: " . $db->lastInsertId());
	} catch (Exception $e) {
...
// DEL buttons hidden for Rust client
		...
		if (get_setting('modsdeladminmsg') == 1) {
			...
			while ($message = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				$msg_class = (strpos($message['text'], 'class="sysmsg"') !== false) ? 'sysmsg' : 'usermsg';
				echo " <span class=\"$msg_class\">$message[text]</span>";
				if ($message['deleted'] == 1) {
					echo ' <small>(deleted)</small>';
				}
				echo "</label></div>";
		...
		while ($message = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			// bhcli.php is for Rust client only - output message directly without wrapper
			echo $message_new;
...
function prepare_message_print(&$message, $removeEmbed)
{
	...
	// Debug: log original message text
	// error_log("[PREPARE_MSG] Original: " . $message['text']);
	
	// Check if this is a PM message
	if (strpos($message['text'], '] to ') !== false || strpos($message['text'], '] - ') !== false) {
		error_log("[PREPARE_MSG] Detected PM or formatted message");
	}
	
	// Strip clickable links from usernames for Rust client compatibility
	// Convert <a ...><span style="...">username</span></a> to just <span style="...">username</span>
	...
	// Flatten double-nested span structure for Rust client parsing
	// Convert <span style="..."><span class="clickablenickname">username</span></span>
	// to just <span style="...">username</span>
	$before = $message['text'];
	$message['text'] = preg_replace(
		'/<span\s+style="([^"]+)"\s*><span\s+class="clickablenickname"\s*>([^<]*)<\/span><\/span>/is',
		'<span style="$1">$2</span>',
		$message['text']
	);
	
	// Also handle case where style span might have other attributes
	$message['text'] = preg_replace(
		'/<span\s+([^>]*style="[^"]+"[^>]*)><span\s+class="clickablenickname"\s*>([^<]*)<\/span><\/span>/is',
		'<span $1>$2</span>',
		$message['text']
	);
	
	// For Rust client: Keep both username and message as span elements
	// The Rust parser's process_node creates child elements for <span> tags
	// After children.reverse(), we need:
	//   children.get(0) = message node
	//   children.get(children.len()-1) = username node
	// So in HTML (before reversal): username span FIRST, message span LAST
	// 
	// Structure must be: <span class="usermsg"><span style="...">username</span> - <span style="...">message</span></span>
	// Both username and message MUST be wrapped in spans to be child elements!
	//
	// No transformation needed - the structure is already correct!
}
